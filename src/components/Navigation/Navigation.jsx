        ...
        !loggedIn ?
          ...
          <div className={`nav-movies ${widthPoint ? `nav-movies_burger` : ''}`}>
            <div className={`nav-movies__wrapper ${widthPoint ? `nav-movies__wrapper_burger` : ''}`}>
              ...
              {widthPoint && <Link className="link-main" to="/" onClick={toggleBurger}>Главная</Link>}

              <Link to="/movies" className={`nav-movies__link ${mainPage ? `nav-movies__link_main` : ''} ${widthPoint ? `nav-movies__link_burger` : ''} ${moviesPath ? `${activeStyleLink}` : ''}`} onClick={toggleBurger}>
                Фильмы
              </Link>

              <Link to="/saved-movies" className={`nav-movies__link  ${mainPage ? `nav-movies__link_main` : ''} ${widthPoint ? `nav-movies__link_burger` : ''} ${savedMoviesPath ? `${activeStyleLink}` : ''}`} onClick={toggleBurger}>
                Сохранённые фильмы
              </Link>
            </div >
